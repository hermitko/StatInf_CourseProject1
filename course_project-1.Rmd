...
Then we sample 40,000 values from \(\mbox{Exp}(0.2)\) and organize it into matrix
...
And let the last evidence the graphics -- the histogram of simulated averages
together with the density of normal distribution (black line).
...
of this tistribution (and with
the Law of large numbers in backpack we are pretty sure, that 40000 samples -- 40 for each
of 1000 simulations -- is well enough).

In case of variance, the story is a little more complicated. The CLT states only that
the density should converge to the density of normal distribution as \(n\) tends to 
infinity. But is \(n=40\) large enough? The personal opinion of the author (as he is 
not very eperienced in statistics) is that for most practical purposes the approximation
is suitable.

There remain some open questions, since mean and variance do not describe the density
in any case fully. As far as author can say -- from the plot is apparent, that the
approximation with normal density is pretty accurate (in the sense of the density curve).
But the proper analysis of this is beyond the scope of this little document (and
beyond the ability of the author, too).

#Notice
The knitr source code of this analysis is available at [github](https://github.com/hermitko/StatInf_CourseProject1).
